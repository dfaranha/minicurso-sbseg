...
%TODO: Figura com clusterizacao obtida pelo EM e os rotulos corretos (com erros destacados, se possível).
...
Em \erick{citar [27]} os autores testaram informação mútua (MIA)\footnote{Mutual Information Analysis.} como um método para localizar vazamento no domínio da frequência e, consequentemente, as bandas de frequência no traces de EM do RSA em que as diferenças entre quadrados e multiplicações são maiores. O Welch $t$-test é um outro método estatístico que pode ser empregado para este fim, p.ex., em metodologias como a TVLA (cf.~\Cref{sec-tvla}). Os autores de \erick{citar [29]} demonstraram como empregar $t$-test e MIA para localizar vazamento no domínio do tempo. 
...
\begin{itemize}
	\item \textbf{Premissa 1}: em um trace $i$, o valor médio para o conjunto de amostras em um índice $m$ que correspondem às iterações cujo bit são 0 ou 1 são, respectivamente, $u_0^i + \gamma_0^i$ e $u_1^i + \gamma_1^i$, onde $\gamma_k^i$ é um ruído aleatório com distribuição normal, $k \in \{0,1\}$.
	\item \textbf{Premissa 2}: para todos os traces $i$, as médias $u_0^i$ e $u_1^i$ são constantes.
\end{itemize}
